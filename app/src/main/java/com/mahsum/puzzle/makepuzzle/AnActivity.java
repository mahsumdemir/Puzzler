...
public class AnActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    try {
      ...
      BitmapMask topLeft = BitmapMask.topLeftMask(getApplicationContext())
          .resizeMask(maskWeight, maskHeight);
      BitmapMask left = BitmapMask.leftMask(getApplicationContext())
          .resizeMask(maskWeight, maskHeight);
      BitmapMask bottomLeft = BitmapMask.bottomLeftMask(getApplicationContext())
          .resizeMask(maskWeight, maskHeight);

      BitmapMask top = BitmapMask.topMask(getApplicationContext())
          .resizeMask(maskWeight, maskHeight);
      BitmapMask center = BitmapMask.centerMask(getApplicationContext())
          .resizeMask(maskWeight, maskHeight);
      BitmapMask bottom = BitmapMask.bottomMask(getApplicationContext())
          .resizeMask(maskWeight, maskHeight);

      BitmapMask topRight = BitmapMask.topRightMask(getApplicationContext())
          .resizeMask(maskWeight, maskHeight);
      BitmapMask right = BitmapMask.rightMask(getApplicationContext())
          .resizeMask(maskWeight, maskHeight);
      BitmapMask bottomRight = BitmapMask.bottomRightMask(getApplicationContext())
          .resizeMask(maskWeight, maskHeight);
      ...
      saveBitmap(bottomRight.maskBitmap(bitmap, 900, 900),
          storage + "/puzzle_images" + "bottomRight");
